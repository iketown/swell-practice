...
type InventoryAsset = {
  id: string;
  assetTag: string;
  assetCodeGroup?: "cable" | "microphone" | "stand" | "instrument" | "pedal" | "rack" | "general";
...
Non-cable gear uses a portrait 1 × 1.5 inch label on the DYMO LabelWriter 450 Turbo. The four-digit inventory number is the dominant element across the 1-inch edge in heavy, tabular sans-serif numerals for fast spoken check-in. The artwork begins below a 4.5 mm blank leading-edge safe zone so the printer does not clip the number when the roll registers slightly before the printable label face. A large QR code below it encodes the full canonical URL `https://theswell.live/g/{four-digit-code}` with level-Q error correction and the standard four-module quiet zone so the iPhone Camera app can recognize it, tolerate modest label damage, and open the check-in page directly. Regular 2.25 mm Arial `www.TheSwell.live` marks run vertically from the divider toward the bottom along both sides, with bold applied only to `TheSwell` and both `www.` and `.live` reduced to 1.85 mm. They are inset 1.25 mm from the label edges with each line's baseline facing its nearest outside edge, and they stay outside the QR quiet zone. The inventory name appears last in up to two smaller compact lines inside a centered 18 mm lane. Printing uses a 25.4 × 38.1 mm page at 100% scale with zero margins.

Implementation status, 2026-08-06: `/gear` also supports batched Code 128 labels on MR610-MAC Letter-size self-laminating stock. The asset editor always offers the canonical `Cables` inventory tag, and only assets carrying that tag are eligible for the sheet-label queue. A compact queue button and pending-label count remain on the Assets tab; the full placement, calibration, print-order, and printing interface opens in a wide scrollable dialog so it does not consume page space between uses. Administrators queue those cable records in print order, choose any of the sheet's 4 rows and 8 columns as the first unused cell, and print the queue across cells in row-major order. Only the 1.00 × 0.752 inch upper box receives ink: a Code 128 barcode encoding the four-digit cable ID sits above one compact line containing the cable title and the same number, such as `6' XLRM → TRSM 0016`. The barcode uses the full horizontal cell width while retaining the required 10-module quiet zone on each side; the text line keeps a small independent inset. The whole block aligns against the laminate boundary, leaving blank space toward the label's open end; the 1.50 inch laminate below remains blank. The print tool includes a plain-paper alignment sheet plus saved horizontal and vertical millimeter offsets so a specific printer can be calibrated without changing code. Existing single-label DYMO 30336 QR and Code 128 printing remains available from each asset editor.
