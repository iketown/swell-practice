...
`/assignments` selects a band, orders its member columns by the band’s unique `voc_1` through `voc_5` defaults, and subscribes to both the song collection and that band’s `bandSongArrangements`. The board grows automatically with the viewport up to its wide desktop maximum, avoiding horizontal scrolling whenever the window can accommodate the full table; narrower viewports retain the table’s own horizontal overflow. Vocal assignments are always visible and editable for administrators: assigned vocal labels are draggable, and a missing assignment renders as a compact empty drop box. Double-clicking an empty vocal box restores that member’s band-default vocal part, moving that part from another member first if necessary to preserve uniqueness. Dropping one assigned part onto another in the same song swaps their parts; dropping it into an empty vocal box moves it there, and dropping it into the shared trash target removes it from the song. Alt/Option-clicking a label toggles its `lead` flag, and any number of parts may be leads. Lead labels use a persistent orange outline without additional text. The legacy `showVocals` compatibility field remains stored as `true` but no longer controls the interface.
...
- Each member column may hold one instrument and one vocal assignment per song.
- The selected band's `bandSongArrangements` documents are the source of truth for the assignment matrix and member pages.
- A member's instrument part comes from that member's ordered matrix column. A member's vocal part comes from the song's explicit vocal assignment or the band's default vocal order when the song has not customized vocals.
- Member pages do not read legacy `memberSongDefaults` or `bandSongOverrides` records and do not show legacy rehearsal assets. Each assigned stem part is a direct link to the corresponding song player.
...
- Moving a part saves a band-only change immediately in `bandSongArrangements`.
- `/members/[memberSlug]` derives the member's instrument and vocal buttons from the selected band's assignment matrix, without loading legacy asset accordions.
...
- A playable `guit_a`, `guit_b`, `bass`, `keys`, `drums`, `guit_acoustic`, `sax`, or `accordion` stem without a matching band-member or Trax assignment appears as a faded, non-draggable icon in the Unassigned column and disappears as soon as its mapped instrument is assigned to either destination.
- A playable `voc_1` through `voc_5` stem without a matching vocal assignment appears as a faded vocal label in the Unassigned column. Administrators can double-click an empty member vocal box to restore that member’s default vocal part.
...
{
  name: string;
  definitionKind: "equipment" | "cable";
  manufacturer?: string;
  model?: string;
  category?: string;
  notes?: string;
...
Implementation status, 2026-08-06: administrators can add shared stage waypoints, append or remove them from a selected cord in route order, drag them on the scaled plot, and add source drop, destination drop, and service-slack allowances. STAGE renders orthogonal physical routes and recalculates required lengths in both STAGE and SIGNAL plus the cable summary. Corridor lane offsets, cable-shortage matching, route reordering controls, and automatic snake suggestions remain later work.
...
type InventoryAsset = {
  id: string;
  assetTag: string;
  assetCodeGroup?: "cable" | "microphone" | "stand" | "instrument" | "pedal" | "rack" | "general" | "container";
  assetCodeVersion?: number; // internal migration marker for the current numeric-ID scheme
  definitionId?: string; // optional for basic inventory-only items such as stands or cases
  tags: string[];
  ...
  expectedContentAssetIds?: string[]; // direct contents; undefined means no packing checklist is configured
...
`assetTag` is the permanent human-facing four-digit inventory ID for every physical or planned asset. IDs are globally unique from `0001` through `9999`, assigned automatically, and not recycled while the asset remains in any lifecycle state, including retired or cancelled. Category bands make numbers easier to recognize: cables use `0001–0099`, microphones begin at `0100`, stands at `0200`, instruments at `0300`, pedals at `0400`, rack gear at `0500`, uncategorized equipment begins at `0600`, and containers use the reserved `1000–1999` series. General allocation skips the container series and resumes at `2000`, so ordinary gear cannot consume new container IDs. The allocator derives the band from the reusable definition category plus the asset name, tags, and container capability, then skips every number already used anywhere in inventory. When inventory loads, every remaining lettered legacy ID is replaced oldest-first within its category. The migration updates the inventory record and its new public label record, purges lettered public ID records, and leaves the asset document ID, history, purchase links, and saved setup assignments unchanged. Old lettered IDs are not retained as aliases because all affected labels will be reprinted.

`purchaseUrl` belongs to the individual inventory asset and is visible only in the admin gear registry and editor. It links directly to the merchant page for that exact purchase and remains separate from a reusable definition's AI research or product-source URL. In inventory rows, this URL appears as a compact **Source** text link directly below the item's thumbnail instead of consuming space in the primary action group. Cable color controls and inventory badges pair the written color name with a colored circular swatch so the list can be scanned visually without relying on color alone.

Cable definitions represent the connector endpoints and other properties shared by that cable type, not a particular length. For example, every `XLRM → TRSM` cable reuses one definition whether it is 6 feet or 10 feet long; `XLRM → XLRF` remains a separate definition because its endpoints differ. Each Cables-tagged inventory asset instead stores its own required `cableLengthInches` value as total inches plus one named `cableColor`. The asset editor reveals linked Feet and Total inches fields only while the Cables tag is present; entering either value converts the other immediately. Labels format that canonical value for people, such as `6'`, while search and inventory display retain the total inches for direct comparisons. Cables never expose or store serial numbers or `stageOnly`: every cable participates in SIGNAL routing, while STAGE placement determines how long the physical cable must be.

The physical-gear registration workflow does not require administrators to leave the owned-item form when the reusable type is missing. The first Gear definition or Cable definition option is **Create definition from this item**. It opens the existing full definition editor in the matching locked mode, including directional ports for equipment or End 1 and End 2 for cables. Saving the definition returns to the still-open asset form, adds the new definition to the catalog, selects it for the item, and preserves the item details already entered. Existing definitions remain the default fast path.

Selectors may persist document IDs and enum keys as their values, but their closed controls always render the corresponding human title or status label. Location selectors show the location name, owner and order-party selectors show the party name, and lifecycle or order-status selectors show their written label. Backend identifiers never appear as user-facing selected text.

The large printed four-digit ID is the primary rapid check-in mark for every asset and remains legible when a self-laminating label wraps around a narrow cable. The authenticated multi-item check-in page accepts rapid numeric entry and pads one to four entered digits automatically, so `23` resolves to `0023` and `100` resolves to `0100`. It also parses local speech-recognition text when every ID is introduced with `gear` or `cable`; for example, `gear 100, gear fifteen, cable oh oh one three` resolves to `0100`, `0015`, and `0013`. Direct speech recognition is progressive enhancement for browsers that expose the phone's speech recognizer, while iPhone keyboard dictation feeds the same parser without uploading a recording to the application server. During listening, every finalized ID is looked up locally and appended to a scrollable review list with its permanent number and inventory label. Each pending row has a large red X to reject it and a large green checkmark to perform its check-in; merely hearing a number never changes its location. Confirmed rows remain visibly marked for the rest of the session, unknown IDs cannot be confirmed, and the focused mobile footer contains only the Start/Stop listening toggle and Exit action.

When an asset's effective power settings have `needsPowerAdapter`, every authenticated check-in surface presents the requirement as `{assetTag} + adapter`, for example `0502 + adapter`. The per-asset setting overrides the reusable definition when present. The adapter carries a second physical label with the same four-digit ID, but it is not a separate inventory record or second check-in event. The stored and machine-readable asset identity remains the bare four digits.

Machine-readable labels preserve the same identity as an optional fallback. A Code 128 cable barcode encodes only the four numeric digits, such as `0016`, while a QR label points to `https://theswell.live/g/{lowercase-asset-tag}`. The phone check-in scanner also accepts a compact Data Matrix containing the bare asset ID, although curved cable labels are not expected to scan reliably. The short route canonicalizes the tag and opens a phone-first landing page. Signed-out visitors can see only the public asset name and tag, send a note to the gear contact, or sign in. An approved administrator can choose from the four most recently used locations, search the full location directory, create a location inline, and append a camera, spoken-list, or numeric check-in. Private inventory details and all location-changing actions remain admin-only.
...
The setup parts panel includes a **Match** view that compares every route-derived cable requirement with active Cables-tagged inventory. Matching treats cable definitions as bidirectional, requires the two connector ends to agree, converts every required distance to inches, assigns longer runs first, and gives each run the shortest remaining cable that reaches it. For example, requirements of 8, 10, and 15 feet against inventory of 5, 10, 10, and 20 feet allocate `8 → 10`, `10 → 10`, and `15 → 20`; the 5-foot cable remains unused. Administrators may apply the full recommendation at once or override each run with another suitable unassigned cable. The chosen asset ID and human-readable label persist on the cable edge.

### 18.6 Containers and Manifests

Implementation status, 2026-08-16: container registration, the reserved 1000-series allocator, nested actual placement, effective-location propagation, cycle/depth validation, container destinations on `/gear/check-in`, and the separate `/gear/pack` workflow are implemented. The check-in destination screen presents parallel Locations and Containers columns, with search and creation controls in each column. The packing screen requires a fresh direct location check-in for the selected container before search or camera entry can add an item. It lists actual direct and nested contents, can set the expected checklist from current direct contents, and reports packed-and-ready, missing, and unexpected states. Corrective actions for discrepancies remain a later slice.
...
- A container's optional `expectedContentAssetIds` list identifies the assets that belong directly in the container. `undefined` means no checklist has been configured; an empty array is an intentionally empty checklist.
- Actual contents are the assets whose latest direct placement is inside that container.
- A later direct check-in of an item to a named location replaces its container placement, clears its inherited container ancestry, and removes it from that container's actual contents.
...
type ContainerAsset = InventoryAsset & {
...
An administrator creating reusable equipment or registering physical gear may paste a public HTTPS product-page URL into the dialog and request research. The server reads the supplied page, extracts product-gallery image candidates, prioritizes technical-specification content, and asks an OpenRouter-hosted model for strict structured equipment data. Sweetwater imports prioritize the product gallery and `Tech Specs`; Amazon imports prioritize the selected variant, feature bullets, `Technical Details`, and `Product Information` while excluding recommendations, reviews, and customer photos. The initial model is `openai/gpt-5.6-terra`, configurable through `OPEN_ROUTER_EQUIPMENT_IMPORT_MODEL`; the credential remains server-only in `OPEN_ROUTER_API_KEY`.
...
- Width, depth, and height normalized to inches, weight normalized to pounds, and the source measurement text used to verify axis interpretation.
- Reference product-photo URLs found in page metadata, structured data, image elements, and supported merchant gallery data.
- Confidence, warnings, and supporting source URLs.

Research never creates or overwrites equipment by itself. The result populates the existing form for administrator review and editing. In `Register physical gear`, approval creates the reviewed reusable definition and physical asset together. The model returns compact port groups, but the trusted server expands every physical connector into an individual `EquipmentPort` record with a stable ID before the draft reaches the editor. The editor operates directly on that exact array and can add another typed bank without flattening mixed connector types into one per-direction default.

Product reference photos remain distinct from both image systems already defined in Section 18.4: they are not the transparent diagram icon and they are not documentary photos of a tagged physical asset. The review gallery preselects up to three candidates and lets the administrator choose the useful images. On approval, the server securely fetches each selected JPEG, PNG, or WebP and the client copies it into Firebase Storage as a reusable definition detail photo, while retaining its merchant source URL for provenance.
...
Setup node details provide an autocomplete across every eligible `inventoryAsset`, including basic assets with no reusable definition. Search matches the asset name, permanent ID, tags, serial number, definition and model, owner, and current location. Exact definition matches sort first, but do not prevent an administrator from assigning a compatible unlinked asset such as a microphone stand. Assets already assigned elsewhere in the setup remain visible with an `Assigned` suffix and identify the setup item using them. Selecting one opens a confirmation dialog; confirming transfers the asset to the current instance and clears the prior node or linked assembly, while cancelling preserves the original assignment. An administrator can also assign an outside provider from `gearParties` or create a planned asset inside the setup modal. The new asset receives its permanent ID immediately, retains `sourceSetupId`, and is assigned to the node without leaving the setup workflow. Template-local owned-unit labels are not part of the current model; owned fulfillment always references a permanent `inventoryAsset` through `assignedAssetId`.

Asset creation assigns the next available four-digit numeric ID automatically from the asset's category series. The ID is read-only in the form and unique across cables and non-cable gear. Cables store their length separately in `cableLengthInches`. Lettered legacy IDs are replaced during migration and are not retained as aliases.
